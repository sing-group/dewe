...
##Volcano plot 
if(image.format == "jpeg") {
	jpeg(paste(workingDirectory, 'volcanoplot.jpeg',sep=""), width = image.width, height = image.height)
} else if(image.format == "tiff") {
	tiff(paste(workingDirectory, 'volcanoplot.tiff',sep=""), width = image.width, height = image.height)
} else if(image.format == "png") {
	png(paste(workingDirectory, 'volcanoplot.png',sep=""), width = image.width, height = image.height)
}
...
plot(results_genes$logfc,-log10(results_genes$pval), col=colors[1], pch=20, xlim=c(-7, 7), xlab="log2(fold change)", ylab="-log10(pValue)") 

# highlight the genes with color  
points(qval_sig_results_genes_plot$logfc,-log10(qval_sig_results_genes_plot$pval), col=colors[2], pch=20) 
points(fc_sig_results_genes_plot$logfc,-log10(fc_sig_results_genes_plot$pval), col=colors[3], pch=20) 
...
legend("bottomright", xjust=1, yjust=1, legend=c("FDR<0.05", "|log2(fold change)|>1", "both"), pch=20, col=c(colors[2],colors[3],colors[4]))
...
if(image.format == "jpeg") {
	jpeg(paste(workingDirectory, 'distr_de_values.jpeg',sep=""), width = image.width, height = image.height)
} else if(image.format == "tiff") {
	tiff(paste(workingDirectory, 'distr_de_values.tiff',sep=""), width = image.width, height = image.height)
} else if(image.format == "png") {
	png(paste(workingDirectory, 'distr_de_values.png',sep=""), width = image.width, height = image.height)
}
...
# draw histogram
hist(results_genes[qsig,"logfc"], breaks=50, col=color, xlab=paste("log2(fold change)",conditions[2], "vs", conditions[1], sep=" "), main="Distribution of differential expression values", xlim=c(-10, 10))
...
if(image.format == "jpeg") {
	jpeg(paste(workingDirectory, 'correlation_plot.jpeg',sep=""), width = image.width, height = image.height)
} else if(image.format == "tiff") {
	tiff(paste(workingDirectory, 'correlation_plot.tiff',sep=""), width = image.width, height = image.height)
} else if(image.format == "png") {
	png(paste(workingDirectory, 'correlation_plot.png',sep=""), width = image.width, height = image.height)
}
...
# label the significant genes
textxy(xfsig,yfsig, cex=.6, labs=row.names(gene_expression[fsig,]))

dev.off()

##Density plot
if(image.format == "jpeg") {
	jpeg(paste(workingDirectory, 'density_plot.jpeg',sep=""), width = image.width, height = image.height)
} else if(image.format == "tiff") {
	tiff(paste(workingDirectory, 'density_plot.tiff',sep=""), width = image.width, height = image.height)
} else if(image.format == "png") {
	png(paste(workingDirectory, 'density_plot.png',sep=""), width = image.width, height = image.height)
}
...
# draw heatmap
if(image.color){
  pheatmap(as.matrix(sig_gene_expression), kmeans_k = NA, scale = "row", clustering_distance_rows = "correlation", clustering_method = "complete",annotation_col = phenotype_table , main="Significant genes", filename=paste(workingDirectory, 'heatmap.', image.format, sep=""),) 
}else{
   group        <- c("gray20", "gray80")
   names(group) <- c(toString(conditions[1]), toString(conditions[2]))
   anno_colors <- list(group = group)
  pheatmap(as.matrix(sig_gene_expression), kmeans_k = NA, scale = "row", clustering_distance_rows = "correlation", clustering_method = "complete",annotation_col = phenotype_table , main="Significant genes", filename=paste(workingDirectory, 'heatmap.', image.format, sep=""), color=gray.colors(20, start = 0.9, end = 0.2, gamma = 2.2, alpha = NULL), annotation_colors = anno_colors)
}
...
if(image.format == "jpeg") {
	jpeg(paste(workingDirectory, 'pca_plot.jpeg',sep=""), width = image.width, height = image.height)
} else if(image.format == "tiff") {
	tiff(paste(workingDirectory, 'pca_plot.tiff',sep=""), width = image.width, height = image.height)
} else if(image.format == "png") {
	png(paste(workingDirectory, 'pca_plot.png',sep=""), width = image.width, height = image.height)
} 
